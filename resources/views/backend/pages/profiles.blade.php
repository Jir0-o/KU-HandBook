    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
                ...
                <div class="px-4 sm:px-0">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            ...
                            Update your system profile information.
                    ...
                    <div class="mt-5">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            <img class="show-image" id="full-logo" src="" alt="Logo">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                ...
                <form id="Edit-System-Submit-Button" enctype="multipart/form-data">
                    <div class="px-4 py-5 bg-white sm:p-6 shadow">
                        <div class="grid grid-cols-6 gap-6">
                            <!-- Current Session  -->
                            <div class="col-span-6 sm:col-span-4">
                                <x-label for="Current Session " value="{{ __('Current Session') }}" />
                                <select class="form-select" wire:model="state.Current Session" 
                                required autocomplete="Current Session" id="as">
                                    <option selected disabled>Please Select</option>
                                </select>
                                <x-input-error for="Current Session" class="mt-2" />
                            </div>

                            <!-- Profile Photo -->
                            <div x-data="{photoName: null, photoPreview: null}" class="col-span-6 sm:col-span-4">
                                <x-label for="photo" value="{{ __('UPLOAD LOGO') }}" />
                                <input type="file" id="logo" name="logo" class="form-control"
                                    x-on:change="photoPreview = URL.createObjectURL($event.target.files[0])" />
                                <x-label for="logo" x-show="!photoPreview" value="{{ __('Existing Logo') }}" />
                                <x-label for="logo" x-show="photoPreview" value="{{ __('Logo') }}" />
                                <!-- Current Profile Photo -->
                                <div class="mt-2" x-show="!photoPreview" id="current-logo"></div>
                        ...
                        <button type="submit" style="float: right;"
                            ...
                            id="Edit-System-Submit-Button">Save</button>
                    </div>
                </form>
            </div>
            <x-section-border />
...
$(document).ready(function() {
    ...
    function fetchApplicableSessions() {
        $.ajax({
            ...
            success: function(response) {
                ...
                response.all.forEach(function(as) {
    ...
    function getSystem() {
        $.ajax({
            ...
            type: "GET",
            success: function(response) {
                $('#as').val(response.data[0].applicable_session_id);
                $('#logo').attr('src', '/global_assets/system_images/' + response.data[0].logo);
                $('#full-logo').attr('src', '/global_assets/system_images/' + response.data[0].logo);
                $('#system_logo').attr('src', '/global_assets/system_images/' + response.data[0].logo);

                $('#current-logo').html('<img src="/global_assets/system_images/' + response.data[0].logo +
                    '" alt="Preview Image" class="rounded-full h-20 w-20 object-cover">'
                );

                systemId = response.data[0].id;
